...
diffmeth <- readRDS(grFile)
diffmeth_hyper = readRDS(grFile_hyper)
diffmeth.gr_hyper = as(diffmeth_hyper, "GRanges")
diffmeth_hypo = readRDS(grFile_hypo)
diffmeth.gr_hypo = as(diffmeth_hypo, "GRanges")
...
# Show number of differentially methylated bases per chromosome
if(length(diffmeth.gr)!=0){
   stats.df = number.of.elements.per.chr(diffmeth.gr)
   datatable(stats.df,
          extensions = 'Buttons',
          options = list(
            dom = 'Brtip',
...
if(length(diffmeth.gr_hyper)!=0){
   stats.df = number.of.elements.per.chr(diffmeth.gr_hyper)
   datatable(stats.df,
          extensions = 'Buttons',
          options = list(
            dom = 'Brtip',
...
if(length(diffmeth.gr_hypo)!=0){
   stats.df = number.of.elements.per.chr(diffmeth.gr_hypo)
   datatable(stats.df,
             extensions = 'Buttons',
          options = list(
            dom = 'Brtip',
...
if(!is.na(fetched.cpgi)){
  ...
  diffCpGann=annotateWithFeatureFlank(diffmeth.gr,
...
# Plotting overview of differentially methylated bases

if(length(diffmeth.gr_hypo)!=0 & length(diffmeth.gr_hyper)!=0){
...
methylDiff.hyper = diffmeth
methylDiff.hyper@.Data[7][[1]]  = rep(80, length(methylDiff.hyper@.Data[6][[1]]))
methylDiff.hypo = diffmeth
methylDiff.hypo @.Data[7][[1]]  = rep(-80, length(methylDiff.hyper@.Data[6][[1]]))

source(ideoDMC_script)

plot( ideoDMC1(methylDiff.hyper, methylDiff.hypo, chrom.length = chr.len,
