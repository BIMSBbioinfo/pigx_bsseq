...
params:
  ...
  assembly: "ce10"
  treatment: [0,1]
  treatment_samples: [0,1]
  sampleids_replic: ["sampleA", "sampleB"]
  mincov: 0
  context: "CpG"
  workdir: "~/outputdir/"
  scripts_dir: ""
...
#covariates <- data.frame(lapply(params$covariates,as.numeric)) # it converts empty strings to NA
covariates <- if(params$covariates==""){NULL}else{data.frame(lapply(params$covariates,as.numeric))}

# change data.frame of strings to numeric
#data.frame(lapply(params$covariates,as.numeric))
saveRDS(covariates, paste0("~/covariates.RDS")) #TODO check what is it when there are no covariates
...
if(!is.null(covariates)){
  inputParams = cbind(inputParams, covariates)
}
...
if(nrow(meth.unite)>1){
      ...
      if(!is.null(covariates)){if(ncol(covariates)+2 >= length(meth.unite@numTs.index)){
          print("Too many covariates/too few replicates. Running differential methylation without covariates.")}
          covariates = NULL
        }
