...
covariates <- if(params$covariates==""){NULL}else{data.frame(lapply(params$covariates,as.numeric))} # it converts empty strings to NA in a data.frame

# change data.frame of strings to numeric
#data.frame(lapply(params$covariates,as.numeric))
saveRDS(covariates, paste0("~/covariates.RDS"))
...
if(!is.null(covariates)){if(ncol(covariates)+2 >= length(sampleids)){
