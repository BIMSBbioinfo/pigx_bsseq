...
rule export_tabix_bigwig:
    input:
        seqlengths=os.path.join(DIR_mapped, ASSEMBLY + "_chromlengths.csv"),
        filepath=os.path.join(DIR_methcall, "{tool}", "tabix_{context}/{prefix}_{context}.txt.bgz"),
    output:
    ...
    message:
        fmt("exporting methylation as bigwig for sample {wildcards.prefix}.")
    shell:
        nice(
            'Rscript',
            [
                "{DIR_scripts}/export_tbx2bw.R",
