...
rule bwameth_genome_preparation:
    ...
    message:
        fmt("Converting {ASSEMBLY} Genome into Bisulfite analogue with bwa-meth")
    shell:
...
rule bwameth_touch_index:
    ...
    message:
        fmt("Update timestamp for {ASSEMBLY} Genome Index")
    shell:
...
rule bwameth_align_trimmed:
    ...
    message:
        fmt("Mapping reads to genome using bwa-meth for sample {wildcards.sample}.")
    shell:
        nice(
            "bwameth",
            [
