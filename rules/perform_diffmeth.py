...
rule unite_meth_calls:
    ...
    params:
        ...
        treatments=lambda wc: ",".join(
            [
                samplesheet(sample, 'Treatment')
                for sample in get_sampleids_from_analysis(wc.analysis)
            ]
        ),
        assembly=ASSEMBLY,
        ...
        min_per_group=lambda wc: config["DManalyses"][wc.analysis].get(
            "minPerGroup",
            config["general"]["differential-methylation"].get("minPerGroup", 0),
        ),
    ...
    message:
        fmt("Uniting samples for differential analysis {wildcards.analysis}")
    shell:
        nice(
            'Rscript',
            [
...
rule diffmeth:
    ...
    input:
        inputfile=os.path.join(
            DIR_diffmeth, "{analysis}", "methylBase_{analysis}_{context}_{tool}.txt.bgz"
        ),
    output:
        methylDiff_tabix_file=os.path.join(
            DIR_diffmeth, "{analysis}", "methylDiff_{analysis}_{context}_{tool}_full.txt.bgz"
        ),
        methylDiff_tabix_index=os.path.join(
            DIR_diffmeth, "{analysis}", "methylDiff_{analysis}_{context}_{tool}_full.txt.bgz.tbi"
        ),
        results_file=os.path.join(
            DIR_diffmeth, "{analysis}", "methylDiff_{analysis}_{context}_{tool}_results.tsv"
        ),
    params:
        sampleids=lambda wc: ','.join(get_sampleids_from_analysis(wc.analysis)),
        treatments=lambda wc: ",".join(
    ...
    message:
        fmt("Calculating differential methylation for analysis {wildcards.analysis}")
    shell:
        nice(
            'Rscript',
            [
...
rule diffmeth_report:
    input:
        methylBase_tabix_file=os.path.join(
            DIR_diffmeth, "{analysis}", "methylBase_{analysis}_{context}_{tool}.txt.bgz"
        ),
        methylDiff_tabix_file=os.path.join(
            DIR_diffmeth, "{analysis}", "methylDiff_{analysis}_{context}_{tool}_full.txt.bgz"
        ),
        methylDiff_results_file=os.path.join(
            DIR_diffmeth, "{analysis}", "methylDiff_{analysis}_{context}_{tool}_results.tsv"
        ),
    ...
    output:
        report=os.path.join(
            DIR_final, "{analysis}", "{analysis}_{context}_{tool}.diffmeth-report.html"
        ),
    params:
    ...
    message:
        fmt("Compiling differential methylation report " + "for analysis " + "{wildcards.analysis}")
    # run:
