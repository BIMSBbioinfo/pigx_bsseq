...
rule methyldackel_extract_methylKit:
    ...
    message:
        fmt(
            "Extract methylation calls from bam file using MethylDackel for sample {wildcards.sample} and protocol {params.protocol}"
        )
    shell:
        nice(
            "methyldackel",
...
rule methyldackel_extract_methylKit_deduped:
    ...
    output:
        cpgCallFile=temp(
            DIR_methcall + "methylDackel/" + "{sample}.deduped_methyldackel_CpG.methylKit"
        ),
    params:
        threads=config['execution']['rules']['methyldackel_extract']['threads'],
    ...
    message:
        fmt(
            "Extract methylation calls from bam file using MethylDackel for sample {{sample}} and protocol {params.protocol}"
        )
    shell:
        nice(
            "methyldackel",
...
rule methyldackel_mbias:
    ...
    message:
        fmt("Calculate methylation bias using MethylDackel for sample {{sample}}.")
    shell:
        nice(
            "methyldackel",
            [
...
rule methyldackel_cytosine_report:
    ...
    message:
        fmt(
            "Extract cytosine report from bam file using MethylDackel for sample {{sample}} and context {params.context}"
        )
    shell:
        nice(
            "methyldackel",
...
rule tabix_methyldackelfile:
    ...
    params:
        ...
        assembly=ASSEMBLY,
        treatment=lambda wc: samplesheet(wc.prefix.replace(".deduped", ""), 'Treatment'),
        context="{context}",
