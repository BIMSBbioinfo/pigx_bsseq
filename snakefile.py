...
targets = {
    ...
    # subsequent rules
    'trimming': {'description': "Trim the reads.", 'files': files_for_sample(list_files_TG)},
...
for target in selected_targets:
    if not target in targets.keys():
        ...
        bail(
            "\n".join(
                [
                    "ERROR: Selected target '{}' is unknown.".format(target),
...
OUTPUT_FILES = list(
    chain.from_iterable(chain.from_iterable([targets[name]['files'] for name in selected_targets]))
)
...
rule final_report:
    input:
        methCall_tabixfile=os.path.join(
    ...
    output:
        report=os.path.join(
            DIR_final, "sample_reports", "{prefix}_{context}_{tool}_{assembly}_final.html"
        ),
    params:
    ...
    message:
        fmt("Compiling final report for sample {wildcards.prefix}.")
    # run:
...
rule methseg:
    ...
    message:
        fmt(
            "Segmenting methylation profile for {wildcards.context} context of sample {wildcards.prefix}."
        )
    shell:
        nice(
            'Rscript',
            [
                "{DIR_scripts}/methSeg.R",
...
rule bam_methCall:
    ...
    output:
        tabixfile=os.path.join(
            DIR_methcall, "methylKit", "tabix_{context}", "{prefix}_{context}.txt.bgz"
        ),
        tabixfileindex=os.path.join(
            DIR_methcall, "methylKit", "tabix_{context}", "{prefix}_{context}.txt.bgz.tbi"
        ),
    params:
        assembly=ASSEMBLY,
        mincov=int(config['general']['methylation-calling']['minimum-coverage']),
    ...
    message:
        fmt("Extract methylation calls from bam file.")
    shell:
        nice(
            'Rscript',
            [
                "{DIR_scripts}/methCall.R",
                "--inBam={input.bamfile}",
                "--assembly={params.assembly}",
                "--mincov={params.mincov}",
                "--minqual={params.minqual}",
...
rule deduplication_se:
    ...
    message:
        fmt("Deduplicating single-end aligned reads from {input}")
    shell:
        nice(
            "samtools",
            [
...
rule deduplication_pe:
    ...
    log:
        DIR_sorted + "{sample}_deduplication.log",
    message:
        fmt("Deduplicating paired-end aligned reads from {input}")
    shell:
        nice(
            "samtools",
            [
...
rule bismark_align_and_map_se:
    ...
    message:
        fmt("Mapping single-end reads to genome {ASSEMBLY}")
    shell:
...
rule bismark_align_and_map_pe:
    ...
    message:
        fmt("Mapping paired-end reads to genome {ASSEMBLY}.")
    shell:
...
rule bismark_genome_preparation:
    ...
    params:
        ...
        pathToBowtie="--path_to_aligner " + os.path.dirname(tool('bowtie2')),
        useBowtie2="--bowtie2 ",
        verbose="--verbose ",
    log:
        os.path.join(OUTDIR, 'bismark_genome_preparation_' + ASSEMBLY + '.log'),
    message:
        fmt("Converting {ASSEMBLY} Genome into Bisulfite analogue")
    shell:
...
rule tabulate_seqlengths:
    ...
    message:
        fmt("Tabulating chromosome lengths in genome: {ASSEMBLY} for later reference.")
    shell:
        nice(
            'samtools',
            [
...
rule fastqc_after_trimming_se:
    ...
    message:
        fmt("Quality checking trimmmed single-end data from sample {wildcards.sample}")
    shell:
...
rule fastqc_after_trimming_pe:
    ...
    log:
        DIR_posttrim_QC + "{sample}_trimmed_fastqc.log",
    message:
        fmt("Quality checking trimmmed paired-end data from sample {wildcards.sample}")
    shell:
        nice('fastqc', ["{params}", "{input}"], "{log}")
...
rule trim_reads_pe:
    input:
        files=[PATHIN + "{sample}_1.fq.gz", PATHIN + "{sample}_2.fq.gz"],
    output:
...
rule fastqc_raw:  #----only need one: covers BOTH pe and se cases.
    ...
    message:
        fmt("Quality checking raw read data from sample {wildcards.sample}")
    shell:
        nice('fastqc', ["{params}", "{input}"], "{log}")
