...
ln -sn ${path_SIGMAT}   ${path_OUT}"/path_links/SIGMAT"    2>/dev/null
...
if [ ${cluster_run} = true ] || [ ${cluster_run} = TRUE ]
  then
    ...
    snakemake -s BSseq_pipeline.py --configfile $path2configfile --cluster-config ${path2clusterconfig}  -d ${path_OUT}  --cluster " qsub -l h_stack={cluster.h_stack}  -l h_vmem={cluster.MEM}  -m abe -M  ${contact_email} -b y  -pe smp {cluster.nthreads} -cwd  "  --jobs ${numjobs} ${snakeparams}
